...
<html>
    ...
    <body>
       <div class="box">
     		<form action="<c:url value="/testPaper_update.action" />" method="post" enctype="multipart/form-data">
     			<input type="hidden" name="testPaperId" value="${testPaper.testPaperId }" />
     			<input type="hidden" name="filePath" id="filePath" value=""/>
     			<table>
     				<tr>
     					<td class="tdName">试卷名称</td>
     					<td class="tdValue"><input type="text" name="paperName" id="paperName" value="${testPaper.paperName }"/></td>
     				...
     				<tr>
     					...
     					<td class="tdValue">
     						...
     						<input type="button" id="upBtn" value="修改文件" onclick="myFile.click()"/>(仅限doc文件)
     				...
     				<tr>
     					...
     					<td class="tdValue">
     					<input type="text" name="totalScore" id="totalScore" value="${testPaper.totalScore }" onkeyup="value=value.replace(/[^\d]/g,'')"/>
     					</td>
     					...
     					<td class="tdValue">
     						...
     						<option></option>
	     						<c:forEach items="${sessionScope.user.gradeSubjectSet }" var="gradeSubject">
	     							<option value="${gradeSubject.gradeSubjectId }" <c:if test='${testPaper.gradeSubject.gradeSubjectId eq gradeSubject.gradeSubjectId }'>selected="selected"</c:if> >
     				...
     				<tr>
     					...
     					<td class="tdValue"><input type="text" name="timeLimit" id="timeLimit" value="${testPaper.timeLimit }" onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
     				...
     				<tr>
     					...
     					<td class="tdValue">
     						超简单 <input type="radio" id="difficulty" name="difficulty" value="1" <c:if test='${testPaper.difficulty eq 1 }'>checked="checked"</c:if> />
     						&nbsp;&nbsp;简单 <input type="radio" id="difficulty" name="difficulty" value="2" <c:if test='${testPaper.difficulty eq 2 }'>checked="checked"</c:if>/>
     						&nbsp;&nbsp;一般 <input type="radio" id="difficulty" name="difficulty" value="3" <c:if test='${testPaper.difficulty eq 3 }'>checked="checked"</c:if>/>
     						&nbsp;&nbsp;困难 <input type="radio" id="difficulty" name="difficulty" value="4" <c:if test='${testPaper.difficulty eq 4 }'>checked="checked"</c:if>/>
     						&nbsp;&nbsp;超困难<input type="radio" id="difficulty" name="difficulty" value="5" <c:if test='${testPaper.difficulty eq 5 }'>checked="checked"</c:if>/>
     					</td>
     				...
     				<tr>
     					...
     					<td class="tdValue">
     						否 <input type="radio" id="isPublic" name="isPublic" value="0" <c:if test='${testPaper.isPublic eq 0 }'>checked="checked"</c:if>/>
     						是 <input type="radio" id="isPublic" name="isPublic" value="1" <c:if test='${testPaper.isPublic eq 1 }'>checked="checked"</c:if>/>
     					</td>
     					<td></td>
     				</tr>     			
     			</table>
